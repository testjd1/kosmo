package chapter04.EX03;

public class Using_Switch {
	public static void main(String[] args) {
/* 	
  	
  	switch 문 - 반드시  break; 사용해서 case 탈출
	해당 case로 점프 한 후에 하위의 모든 case를 실행
	break;  -> 해당 실행문 빠져나옴
 */
		
/*		
 	
// 1) switch 구문에서 break; 를 사용하지 않은 경우
  
		int a = 3;
		
		switch(a) { //(위치 변수 ) : 정수, 문자, 문자열
		case 1:
			System.out.println("변수 값을 출력 " + a);
		case 2:
			System.out.println("변수 값을 출력 " + a);
		case 3:
			System.out.println("변수 값을 출력 " + a);
		case 4 :
			System.out.println("변수 값을 출력 " + a);
		default:
			System.out.println("변수 값을 출력 " + a);
		}
*/
/*	
// 2) switch 구문에서 break; 를 사용하는 경우
		int a = 3;
		
		switch(a) { //(위치 변수 ) : 정수, 문자, 문자열
		case 1:
			System.out.println("case 1을 출력 " + a);
			break;
		case 2:
			System.out.println("case 2을 출력 " + a);
			break;
		case 3:
			System.out.println("case 3을 출력 " + a);
			break;
		case 4 :
			System.out.println("case 4을 출력 " + a);
			break;
		default:
			System.out.println("default를 출력 " + a);
			// default  에서  break는 일반적으로 생략
			
		}
*/		
		
/*	
 	
	// 7점 이상이면 pass, 그렇지 않으면 fail
		int b = 8;
		
		switch (b) {
		case 10:
			System.out.println("Pass");
			break;
		case 9:
			System.out.println("Pass");
			break;
		case 8:
			System.out.println("Pass");
			break;
		case 7:
			System.out.println("Pass");
			break;
		default:
			System.out.println("Fail");
		}
*/	
/*		
		// 축약 표현
		 int b = 8;
		 switch (b) {
		 case 10: case 9: case 8: case 7:
			 System.out.println("Pass");
			 break;
		 default:
			 System.out.println("Fail");
		 }
*/		
		
		 // switch <-> if~ else if문과 상호 변환 가능
		
		int c = 5;
		
		switch(c) {
		case 10: case 9:
			System.out.println("A 학점");
			break;
		case 8:
			System.out.println("B 학점");
			break;
		case 7:
			System.out.println("C 학점");
			break;
		default:
			System.out.println("D 학점");
		}
		
		// 위 구문을 if ~ else if  로 변환
		
		c = 7;
		if(c > 9) {
		}else if (c == 8) {
			System.out.println("B 학점");
		}else if (c == 7) {
			System.out.println("C 학점");
		}else {
			System.out.println("D 학점");
		}
	}	
}
